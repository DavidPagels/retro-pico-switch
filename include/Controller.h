...
class Controller {
 public:
  ...
  void updatePioOutputSize(uint8_t autoPullLength) {
    ...
    _pio->sm[_sm].shiftctrl = (_pio->sm[_sm].shiftctrl & 0xA00FFFFF) |
                              (0x8 << 20) |
  ...
  void sendData(uint8_t *request, uint8_t dataLength, uint8_t *response,
  ...
  virtual void init() = 0;
  virtual void getSwitchUsbReport(SwitchUsbReport *switchUsbReport) = 0;
  virtual void getSwitchBtReport(SwitchBtReport *switchBtReport) = 0;
